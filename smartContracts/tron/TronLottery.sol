...
contract TronLottery {
    uint public constant TICKET_PRICE = 0x01; // 1e16 = 0.01 ether
...
contract RecurringLottery {
    ...
    function RecLottery (uint _duration) public {
        duration = _duration;
        round = 1;
...
contract RNGLottery {
    ...
    function RLottery (uint duration, uint revealDuration) public {
...
contract Powerball {
    ...
    function drawNumbers (uint _round) public {
        ...
        uint seed = 0;
        while (i < 5) {
            bytes32 rand = keccak256(abi.encodePacked(blockhash(drawBlock), seed));
    ...
    function winningNumbersFor(uint _round) public view
      returns (uint[6] memory winningNumbers) {
